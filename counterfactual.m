...
% Идея: по итогам байесовской оценки мы имеем 1000 семплов или 1000
% "моделей", каждая из которых соответствует нашему prior, данным, знаковым
% ограничениям. 
% 
% I. На их основе мы можем построить 1000 контртфактических
% симуляций, рекурсивно пересчитывая шоки и значения переменных. Из 1000
% контрфактических симуляций мы затем строим медианную траекторию (логика 
% такая же, как в построении импульсных откликов). Выполняются симуляции по
% 2 сценариям: 
% 1) если бы ключевая ставка была зафиксирована на уровне 7,5% с 2023к3
% 2) если бы шоки ДКП были равны 0 с 2023к3 (ключевая ставка по Тейлору)
...
% Для 1 сценария рассчитываются дополнительные прогнозы при траектории 
% ключевой ставки, равной консенсус-прогнозу, и на основе
% ожиданий из кривой бескупонной доходности Московской биржи. 
% Для 2 и 3 сценариев - только ставки из кривой бескупонной доходности. 

% NB! Тут не оценивается модель. Она уже оценена. 
